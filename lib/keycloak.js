...
// Make sha256 available globally for fallback
...
export default class Keycloak {
    ...
    async init(initOptions = {}) {
        ...
        const adapters = ['default', 'cordova', 'cordova-native', 'capacitor-native'];
        ...
        if (typeof initOptions.adapter === 'string' && adapters.includes(initOptions.adapter)) {
            ...
            this.#adapter = this.#loadAdapter('capacitor-native');
    ...
    /**
     * @param {"default" | "cordova" | "cordova-native" | "capacitor-native"} type
    ...
    #loadAdapter(type) {
        ...
        if (type === 'capacitor-native') {
            this.#loginIframe.enable = false;
            return this.#loadCapacitorNativeAdapter();
        }
    ...
    #loadDefaultAdapter() {
        ...
        const redirectUri = (options) => {
            const uri = options?.redirectUri || this.redirectUri || location.href;
            return uri;
        }
    ...
    #loadCordovaAdapter() {
        ...
        var getCordovaRedirectUri = () => {
            const uri = this.redirectUri || 'http://localhost';
            return uri;
        }
        ...
        return {
            login: async (options) => {
                ...
                await new Promise(async (resolve, reject) => {
                    const timeout = setTimeout(async () => {
                        await Browser.close();
                        reject(new Error('Login timeout: No redirect received.'));
                    }, 600000);
                    ...
                    const listener = await App.addListener('appUrlOpen', async ({url}) => {
                        ...
                        try {
                            const oauth = this.#parseCallback(url);
                            await this.#processCallback(oauth);
                            resolve();
                        } catch (err) {
                            reject(err);
            ...
            register: async (options) => {
                ...
                await new Promise(async (resolve, reject) => {
                    const timeout = setTimeout(async () => {
                        await Browser.close();
                        reject(new Error('Registration timeout: No redirect received.'));
                    }, 600000);
                    ...
                    await Browser.open({url: registerUrl});
                });
            },
            ...
            redirectUri: (options) => {
                const uri = options.redirectUri || this.redirectUri || "http://localhost";
                return uri;
            }
        }
    }
    ...
    createLogoutUrl(options) {
        ...
        console.log("redirectUri", this.#adapter.redirectUri(options))
        const params = new URLSearchParams([
            ['client_id', /** @type {string} */ (this.clientId)],
...
async function generatePkceChallenge(pkceMethod, codeVerifier) {
    ...
    // Try native Web Crypto API first
    ...
    // Fallback: use js-sha256 if available
    if ((typeof window !== 'undefined' && window.sha256) || 
        (typeof globalThis !== 'undefined' && globalThis.sha256)) {
        // js-sha256 returns hex, so we need to convert to bytes then base64url
        const sha256Fn = (typeof window !== 'undefined' && window.sha256) ? window.sha256 : globalThis.sha256;
        const hex = sha256Fn(codeVerifier);
        const bytes = hexToBytes(hex);
        const encodedHash = bytesToBase64(bytes)
            .replace(/\+/g, '-')
            .replace(/\//g, '_')
            .replace(/=/g, '');
        return encodedHash;
    ...
    // Fallback: use a minimal inline SHA-256 polyfill (not recommended for production)
    throw new Error("No available SHA-256 implementation for PKCE challenge. Please include 'js-sha256' or run in a secure context.");
}
